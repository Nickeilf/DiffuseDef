...
class GreedyWordSwapWIRPWWS(SearchMethod):
    ...
    def _get_index_order(self, initial_text):
        ...
        for idx in range(len_text):
            ...
            temp_dict["saliency_score"] = (
                swap_results[max_id].score * softmax_saliency_scores[idx]
            )
            index_and_word.append(temp_dict)
    ...
    def _perform_search(self, initial_result):
        ...
        while i < len(index_and_word) and not search_over:
            if index_and_word[i]["word"] is None:
                break
            transformed_text_candidate = cur_attacked_text.replace_word_at_index(
                index=index_and_word[i]["idx"], new_word=index_and_word[i]["word"]
            )
            i += 1
            # need to give last_transformation, but cannot find an API to get it from here
            transformed_text_candidate.attack_attrs["last_transformation"] = (
                self.transformation
            )
            transformed_text_candidate_filtered = self.filter_transformations(

...
class FasterAugmenter:
    ...
    def augment(self, text):
        ...
        for _ in range(self.transformations_per_example):
            ...
            transformed_modify_indices = [
                list(text.attack_attrs["modified_indices"])[0]
                for text in transformed_texts
            ]
            transformed_modify_items = defaultdict(list)
            for modify_index, text in zip(
                transformed_modify_indices, transformed_texts
            ):
            ...
            if len(transformed_modify_items.keys()) <= num_words_to_swap:
                ...
                chosen_indices = np.random.choice(
                    list(transformed_modify_items.keys()),
                    size=(num_words_to_swap,),
                    replace=False,
                ).tolist()

            chosen_transformed_texts = [
                random.choice(transformed_modify_items[index])
                for index in chosen_indices
            ]
            for text, index in zip(chosen_transformed_texts, chosen_indices):
                current_text = current_text.replace_word_at_index(

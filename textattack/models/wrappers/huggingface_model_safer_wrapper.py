    ...
    def __call__(self, text_input_list):
        ...
        if isinstance(text_input_list[0], tuple) and len(text_input_list[0]) == 2:
            for text_input in text_input_list:
                randomized_text_input = [
                    (text_input[0], sentence)
                    for sentence in self.augmentor.get_perturbed_batch(
                        text_input[1].lower(), rep=self.ensemble
                    )
                ]
                randomized_text_input_list.extend(randomized_text_input)
        else:
            for text_input in text_input_list:
                randomized_text_input_list.extend(
                    self.augmentor.get_perturbed_batch(
                        text_input.lower(), rep=self.ensemble
                    )
                )

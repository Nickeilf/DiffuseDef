...
class HuggingFaceModelMaskEnsembleWrapper(PyTorchModelWrapper):
    ...
    def mask_lm_loss(
        self, ids: torch.Tensor, pred: torch.Tensor, delete_special_tokens: bool = True
    ) -> torch.Tensor:
    ...
    def get_tokenizer_mapping_for_sentence(self, sentence: str) -> Tuple:
        if isinstance(self.lm_tokenizer, RobertaTokenizer):
            ...
            enc_result = [
                self.lm_tokenizer.encode(sentence_tokens[0], add_special_tokens=False)
            ]
            enc_result.extend(
                [
                    self.lm_tokenizer.encode(
                        x, add_special_tokens=False, add_prefix_space=True
                    )
                    for x in sentence_tokens[1:]
                ]
            )
        else:
            enc_result = [
                self.lm_tokenizer.encode(x, add_special_tokens=False)
    ...
    def mask_probs(self, sentence: str) -> List[float]:
        encodings = self.lm_tokenizer.encode_plus(
            sentence,
            truncation=True,
    ...
    def __call__(self, text_input_list):
        ...
        if isinstance(text_input_list[0], tuple) and len(text_input_list[0]) == 2:
            for text_input in text_input_list:
                mask_text_input = [
                    (text_input[0], sentence)
                    for sentence in self.mask_sentence_decrator(text_input[1])
                ]
        ...
        ensemble_logits_for_each_input = np.split(
            outputs, indices_or_sections=len(text_input_list), axis=0
        )
        logits_list = []

...
class SeqClsWrapper(nn.Module):
    ...
    def get_encoder_outputs(
        self, input_ids, attention_mask, labels=None, delta_grad=None, grad_idx=None
    ):
        """
        ...
        if delta_grad is not None:
            ...
            outputs = self.transformer_forward(
                inputs_embeds=emb_grad, attention_mask=attention_mask
            )
        else:
            outputs = self.transformer_forward(
    ...
    def grad_mask(self, input_ids, attention_mask, pred=None, mask_filter=True):
        ...
        for i, len_ in enumerate(b_length):
            if len_ > 10:
                ...
                val, indices_ = torch.topk(norm_grad[i, :len_], 1)
            ...
            if mask_filter:
                ind = [
                    x.item()
                    for x in indices_
                    if x <= len_.item() and x != last_token and x != 0
                ]
            else:
    ...
    def two_step_ensemble(
        self, input_ids, attention_mask, mask_indices, num_ensemble=5, binom_ensemble=50
    ):
        logit_score_list = []
        ...
        prediction_score = torch.zeros(input_ids.size(0), self.num_classes).to(
            self.device
        )
        ...
        pred_max_logit = torch.zeros(input_ids.size(0), self.num_classes).to(
            self.device
        )
        ...
        for j, n_a_ in enumerate(n_a):
            p_val = binomtest(n_a_, num_ensemble, p=0.5, alternative="less").pvalue
            if p_val < self.alpha_p:
                input_ids_ = input_ids[j].unsqueeze(0)
                attn_m = attention_mask[j].unsqueeze(0)

                mask_ind, _ = self.grad_mask(
                    input_ids_, attn_m, pred=None, mask_filter=True
                )
                self.model.zero_grad()
                ...
                if self.vote_type == "max":
                    max_vote = np.bincount(
                        output_re["logits"].argmax(dim=-1).tolist(),
                        minlength=self.num_classes,
                    )
                    max_pred = max_vote.argmax()
                    re_logit_ = torch.zeros(input_ids_.size(0), self.num_classes).to(
                        self.device
                    )

...
class Metric(ABC):
    ...
    def __str__(self):
        return ", ".join(
            [
                "{}: {:.4f}".format(key, value)
                for (key, value) in self.get_metric().items()
            ]
        )
...
class ClassificationMetric(Metric):
    ...
    def __call__(
        ...
        self._all_losses = torch.cat(
            [self._all_losses, losses.reshape(1).to(self._all_losses.device)], dim=0
        )
        ...
        self._all_gold_labels = torch.cat(
            [self._all_gold_labels, gold_labels.to(self._all_gold_labels.device)], dim=0
        )
    ...
    def get_metric(self, reset: bool = False) -> Dict:
        ...
        accuracy = (
            torch.sum(self._all_gold_labels == self._all_predictions).item() / total_num
        )
        f1 = f1_score(
            y_true=self._all_gold_labels.numpy(),
            y_pred=self._all_predictions.numpy(),
            average="macro",
        )
        result = {"loss": loss, "accuracy": accuracy, "f1": f1}
        if reset:
            self.reset()
        return result
...
class RandomSmoothAccuracyMetrics(Metric):
    ...
    def get_metric(self, reset: bool = False) -> Dict:
        result = {
            "accuracy": self._correct_numbers / self._all_numbers,
...
class RandomAblationCertifyMetric(Metric):
    ...
    def get_metric(self, reset: bool = False) -> Dict:
        ...
        radius_rate = [
            radius / length
            for radius, length in zip(self._certify_radius, self._sentence_length)
        ]
        result = {
            "accuracy": sum(~np.isnan(self._certify_radius))
            / len(self._certify_radius),
            "median": np.median(
                [-1 if np.isnan(radius) else radius for radius in self._certify_radius]
            ),
            "median(right)": np.nanmedian(self._certify_radius),
            "mean": np.nanmean(self._certify_radius),
            "median rate": np.median(
                [-1 if np.isnan(rate) else rate for rate in radius_rate]
            ),
    ...
    def format_list(self, lst: List, num_for_row: int = 50) -> str:
        return "\n".join(
            [
                " ".join(self.to_str(lst[i : i + num_for_row]))
                for i in range(0, len(lst), num_for_row)
            ]
        )

    def certify_radius(self) -> str:
        return "Certify Radius List : \n{}".format(
            self.format_list(self._certify_radius)
        )
